...
void handleClient(SOCKET clientSocket) { // Hàm xử lý dữ liệu từ client
    ...
    if (strcmp(method, "CONNECT") == 0) { // Nếu method là CONNECT
        ...
        cout << "hostname: " << hostname << endl; // In ra hostname
        cout << "url: " << url << endl; // In ra url
...
void proxy_server(int argc, char *argv[]) {

    if (argc > 1) {
        for(int i = 1; i <= argc; i++) {
            ban_list.push_back(argv[i]); // Thêm các trang web vào ban_list
        }
    }
    ...
    while (true) { // Lặp để chấp nhận kết nối từ client
        ...
        thread t(handleClient, clientSocket); // Tạo một thread để xử lý dữ liệu từ client
        //cerr << "Thread created\n";
        t.detach(); // Tách thread
